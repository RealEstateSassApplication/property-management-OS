...
func (f *fakeRepository) CreatePending(_ context.Context, document Document) (Document, error) {
	f.document = document
	return document, nil
}
func (f *fakeRepository) Get(_ context.Context, _, _ string) (Document, error) {
	if f.document.ID == "" {
		return Document{}, ErrNotFound
	}
	return f.document, nil
}
func (f *fakeRepository) List(context.Context, string, Filter) ([]Document, error) {
	return []Document{f.document}, nil
}
func (f *fakeRepository) ResourceExists(context.Context, string, string, string) (bool, error) {
	return f.exists, nil
}
func (f *fakeRepository) UpdateStatus(_ context.Context, _, _, _, status string, verifiedAt, deletedAt *time.Time) (Document, error) {
	f.document.Status = status
	f.document.VerifiedAt = verifiedAt
	f.document.DeletedAt = deletedAt
	return f.document, nil
}
...
func (f fakeStorage) PresignPut(context.Context, string, string, time.Duration) (string, map[string]string, error) {
	return "https://upload.example", map[string]string{"Content-Type": "application/pdf"}, f.err
}
...
func (f fakeStorage) PresignGet(context.Context, string, time.Duration) (string, error) {
	return "https://download.example", f.err
}
...
func TestInitiateUploadNormalizesFileAndCreatesPendingDocument(t *testing.T) {
	...
	if intent.Document.FileName != "Signed_Lease_2026.pdf" {
		t.Fatalf("unexpected filename %q", intent.Document.FileName)
	}
	if intent.Document.Status != "pending" || intent.UploadURL == "" {
		t.Fatalf("expected pending upload intent: %+v", intent)
	}
}
...
func TestCompleteUploadQuarantinesMetadataMismatch(t *testing.T) {
	...
	if repository.document.Status != "quarantined" {
		t.Fatalf("expected quarantined, got %s", repository.document.Status)
	}
}
...
func TestDownloadRequiresAvailableDocument(t *testing.T) {
	...
	if !errors.Is(err, ErrDocumentNotAvailable) {
