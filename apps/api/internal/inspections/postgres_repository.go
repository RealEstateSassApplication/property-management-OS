...
func (r *PostgresRepository) List(ctx context.Context, organizationID string) ([]Inspection, error) {
	...
	for rows.Next() {
		var item Inspection
		if err := scanInspection(rows, &item); err != nil { return nil, err }
...
func (r *PostgresRepository) Get(ctx context.Context, organizationID, inspectionID string) (Inspection, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Inspection{}, ErrInspectionNotFound }
	return item, err
}
...
func (r *PostgresRepository) ListItems(ctx context.Context, organizationID, inspectionID string) ([]Item, error) {
	...
	for rows.Next() {
		var item Item
		if err := rows.Scan(&item.ID,&item.OrganizationID,&item.InspectionID,&item.Area,&item.ItemName,&item.Condition,&item.Notes,&item.EvidenceDocumentID,&item.CreatedAt,&item.UpdatedAt); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (r *PostgresRepository) Create(ctx context.Context, organizationID, actorUserID string, input CreateInspectionInput) (Inspection, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Inspection{}, ErrTenancyNotFound }
	if err != nil { return Inspection{}, err }
	var scheduled any
	...
	if err := tx.QueryRow(ctx, `INSERT INTO inspections (organization_id,tenancy_id,property_id,unit_id,inspection_type,status,scheduled_for,summary,created_by_user_id) VALUES ($1,$2,$3,$4,$5,'draft',$6,NULLIF($7,''),NULLIF($8,'')::uuid) RETURNING id`, organizationID,input.TenancyID,propertyID,unitID,input.InspectionType,scheduled,input.Summary,actorUserID).Scan(&inspectionID); err != nil { return Inspection{}, err }
	if _, err := tx.Exec(ctx, `INSERT INTO audit_events (organization_id,actor_user_id,action,resource_type,resource_id,metadata) VALUES ($1,NULLIF($2,'')::uuid,'inspection.created','inspection',$3,jsonb_build_object('type',$4,'tenancyId',$5))`, organizationID,actorUserID,inspectionID,input.InspectionType,input.TenancyID); err != nil { return Inspection{}, err }
	if err := tx.Commit(ctx); err != nil { return Inspection{}, err }
...
func (r *PostgresRepository) CreateItem(ctx context.Context, organizationID string, input CreateItemInput) (Item, error) {
	...
	if err := r.pool.QueryRow(ctx, `SELECT status FROM inspections WHERE organization_id=$1 AND id=$2`, organizationID,input.InspectionID).Scan(&status); err != nil {
		if errors.Is(err, pgx.ErrNoRows) { return Item{}, ErrInspectionNotFound }
		return Item{}, err
	}
	if status == "completed" || status == "acknowledged" || status == "cancelled" { return Item{}, ErrInspectionClosed }
	if input.EvidenceDocumentID != "" {
		...
		if err := r.pool.QueryRow(ctx, `SELECT status FROM documents WHERE organization_id=$1 AND id=$2`, organizationID,input.EvidenceDocumentID).Scan(&docStatus); err != nil {
			if errors.Is(err, pgx.ErrNoRows) { return Item{}, ErrDocumentNotFound }
			return Item{}, err
		}
		if docStatus != "available" { return Item{}, ErrDocumentNotAvailable }
	}
	var item Item
...
func (r *PostgresRepository) Complete(ctx context.Context, organizationID, actorUserID string, input CompleteInspectionInput) (Inspection, error) {
	...
	if status == "completed" || status == "acknowledged" || status == "cancelled" { return Inspection{}, ErrInspectionClosed }
	var count int
	if err := tx.QueryRow(ctx, `SELECT count(*) FROM inspection_items WHERE organization_id=$1 AND inspection_id=$2`, organizationID,input.InspectionID).Scan(&count); err != nil { return Inspection{}, err }
	if count == 0 { return Inspection{}, ErrInspectionHasNoItems }
	if _, err := tx.Exec(ctx, `UPDATE inspections SET status='completed',summary=COALESCE(NULLIF($3,''),summary),completed_at=now(),completed_by_user_id=NULLIF($4,'')::uuid,updated_at=now() WHERE organization_id=$1 AND id=$2`, organizationID,input.InspectionID,input.Summary,actorUserID); err != nil { return Inspection{}, err }
	if _, err := tx.Exec(ctx, `INSERT INTO audit_events (organization_id,actor_user_id,action,resource_type,resource_id,metadata) VALUES ($1,NULLIF($2,'')::uuid,'inspection.completed','inspection',$3,jsonb_build_object('itemCount',$4))`, organizationID,actorUserID,input.InspectionID,count); err != nil { return Inspection{}, err }
	if err := tx.Commit(ctx); err != nil { return Inspection{}, err }
...
func (r *PostgresRepository) Acknowledge(ctx context.Context, organizationID, actorUserID string, input AcknowledgeInspectionInput) (Inspection, error) {
	...
	if result.RowsAffected() == 0 {
		var status string
		if err := r.pool.QueryRow(ctx, `SELECT status FROM inspections WHERE organization_id=$1 AND id=$2`, organizationID,input.InspectionID).Scan(&status); errors.Is(err, pgx.ErrNoRows) { return Inspection{}, ErrInspectionNotFound } else if err != nil { return Inspection{}, err }
