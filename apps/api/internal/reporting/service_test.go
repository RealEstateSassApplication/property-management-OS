...
func (f *fakeReportingRepository) Dashboard(context.Context, string) (Dashboard, error) { return f.dashboard, nil }
...
func TestAuditLimitDefaultsAndCaps(t *testing.T) {
	...
	service := NewService(repo)
	if _, err := service.ListAudit(context.Background(), "org", 0); err != nil { t.Fatal(err) }
	if repo.limit != 100 { t.Fatalf("expected default 100, got %d", repo.limit) }
	if _, err := service.ListAudit(context.Background(), "org", 9999); err != nil { t.Fatal(err) }
	if repo.limit != 500 { t.Fatalf("expected cap 500, got %d", repo.limit) }
}
